...
function createSpPicElement(src) {
	...
	pictureImg.src = chrome.extension.getURL(src);

	span.appendChild(pictureImg);
		...
		fileInput.onchange = () => {
			...
			if (file.type.includes('image')) {
				...
				tmpImg.onload = () => {
					...
					pictureImg.src = iconEncodeURL(ctx);
				};
...
function createSearchProviderElement(name = '', Picture = 'Pictures/other.png', url = '', chosen = false, edit = true) {
	const root = $el('div');
	...
	root.appendChild(createSpPicElement(Picture));
...
restoreDefaultSearchProviders.onclick = () => {
	...
	for (const p of backgroundPage.getDefaultProvidersClone()) {
		searchProviderList.appendChild(createSearchProviderElement(p.name, p.Picture, p.url, p.chosen, false));
	}
};
...
saveOptions.onclick = () => {
	...
	for (const li of searchProviderList.children) {
		...
		const Picture = li.children[1].firstElementChild.src.replace(extensionUUID, '');
		...
		storedSettings.storageProviders.push(new Provider(name, Picture, url, chosen));
		nameSet.add(name);
	}
